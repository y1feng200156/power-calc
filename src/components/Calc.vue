        ...
        <v-container>
              ...
              <v-card class="elevation-8">
                ...
                <v-list dense>
                  <v-list-tile>
                    ...
                    >{{ monthTotal }}</v-list-tile-content>
                  </v-list-tile>
                  <v-list-tile>
                    ...
                    <v-list-tile-content
                      class="align-end"
                      style="align-items: flex-end;"
                    >￥{{ monthPrice }}</v-list-tile-content>
                  </v-list-tile>
                  <v-list-tile>
          ...
          <v-layout column align-center justify-center>
            <div class="subheading text-xs-center">
              <code>以上的计算结果只是估算，与实际使用中有出入。</code>
            </div>
          </v-layout>
        </v-container>
      </v-parallax>
          ...
          <v-flex mt-2 mb-2 xs12 sm8 offset-sm2>
            <v-card class="elevation-2">
              <v-card-title class="elevation-2">
                <h4>{{ $vuetify.t('$vuetify.powerLadderDesc') }}</h4>
              </v-card-title>
              <v-divider/>
              <v-list dense>
                <template v-for="(item, index) in prices">
                  <v-subheader :key="index">{{ item.title }}</v-subheader>
                  <template v-for="(price, pIndex) in item.values">
                    <v-list-tile avatar :key="`${index}-${pIndex}`">
                      <v-list-tile-content
                        :key="`${index}-${pIndex}-content`"
                      >{{$vuetify.t('$vuetify.ladder', pIndex + 1)}}</v-list-tile-content>
                      <v-list-tile-content
                        :key="`${index}-${pIndex}-sub-content`"
                        class="align-end"
                        style="align-items: flex-end;"
                      >
                        <span
                          class="text--primary"
                        >{{ $vuetify.t('$vuetify.humanPrice', price) }}/{{ $vuetify.t('$vuetify.powerUnit') }}</span>
                        <span class="text--secondary">
                          <template
                            v-if="pIndex === 0"
                          >{{ $vuetify.t('$vuetify.ladderPower.first', item.powers[pIndex]) }}</template>
                          <template
                            v-if="pIndex > 0 && pIndex + 1 < item.values.length"
                          >{{ $vuetify.t('$vuetify.ladderPower.center', item.powers[pIndex - 1], item.powers[pIndex]) }}</template>
                          <template
                            v-if="pIndex + 1 === item.values.length"
                          >{{ $vuetify.t('$vuetify.ladderPower.end', item.powers[pIndex - 1]) }}</template>
                        </span>
                      </v-list-tile-content>
                    </v-list-tile>
                    <v-divider
                      v-if="pIndex + 1 < item.values.length"
                      :key="`${index}-${pIndex}-divider`"
                      inset
                    />
                  </template>
...
export default class Calc extends Vue {
  ...
  public created() {
    this.$ajax('/power_bases').then((response: any) => {
      this.prices = response.data;
      this.form.prices = response.data[0];
    });
  }
}
</script>
